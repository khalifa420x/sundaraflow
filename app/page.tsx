    ...
    <div className="sf-root">
      ...
      <nav className="sf-nav">
        <div className="sf-nav-inner">
          ...
          <button className="sf-burger" onClick={() => setMenuOpen(v => !v)} aria-label="Menu"><span /><span /><span /></button>
        </div>
        {menuOpen && (
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        .sf-root { background: #121212; color: #FFFFFF; font-family: 'Inter', sans-serif; overflow-x: hidden; min-height: 100vh; }

        /* NAV */
        .sf-nav { position: fixed; top: 0; left: 0; right: 0; z-index: 200; background: rgba(18,18,18,.88); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border-bottom: 1px solid rgba(255,255,255,.06); }
        .sf-nav-inner { max-width: 1200px; margin: 0 auto; padding: 0 28px; height: 64px; display: flex; align-items: center; gap: 32px; }
        .sf-logo { font-family: 'Lexend', sans-serif; font-weight: 900; font-size: 1.15rem; letter-spacing: .04em; cursor: pointer; text-transform: uppercase; color: #FFFFFF; flex-shrink: 0; }
        .sf-logo span { color: #9E1B1B; }
        .sf-nav-links { display: flex; gap: 28px; flex: 1; justify-content: center; }
        .sf-nav-links a { font-size: .78rem; font-weight: 500; color: rgba(255,255,255,.6); letter-spacing: .06em; text-transform: uppercase; text-decoration: none; transition: color .2s; }
        .sf-nav-links a:hover { color: #FFFFFF; }
        .sf-nav-ctas { display: flex; align-items: center; gap: 10px; margin-left: auto; }
        .sf-burger { display: none; flex-direction: column; gap: 5px; background: transparent; border: none; cursor: pointer; padding: 4px; margin-left: auto; }
        .sf-burger span { display: block; width: 22px; height: 2px; background: rgba(255,255,255,.7); border-radius: 2px; }
        .sf-mobile-menu { display: flex; flex-direction: column; gap: 12px; padding: 18px 24px 24px; border-top: 1px solid rgba(255,255,255,.06); background: #121212; }
        .sf-mobile-menu a { font-size: .82rem; color: rgba(255,255,255,.7); text-decoration: none; letter-spacing: .06em; text-transform: uppercase; }

        /* BUTTONS */
        .sf-btn-primary { display: inline-flex; align-items: center; justify-content: center; gap: 6px; padding: 11px 24px; background: linear-gradient(135deg, #9E1B1B, #7a1212); border: none; border-radius: 9999px; color: #FFFFFF; font-family: 'Lexend', sans-serif; font-weight: 800; font-size: .75rem; letter-spacing: .1em; text-transform: uppercase; cursor: pointer; transition: box-shadow .25s, transform .15s; }
        .sf-btn-primary:hover { box-shadow: 0 0 36px rgba(158,27,27,.45); transform: translateY(-1px); }
        .sf-btn-ghost { display: inline-flex; align-items: center; justify-content: center; padding: 11px 22px; background: transparent; border: 1px solid rgba(255,255,255,.15); border-radius: 9999px; color: rgba(255,255,255,.75); font-family: 'Lexend', sans-serif; font-weight: 700; font-size: .75rem; letter-spacing: .1em; text-transform: uppercase; cursor: pointer; transition: border-color .2s, color .2s; }
        .sf-btn-ghost:hover { border-color: rgba(255,255,255,.4); color: #FFFFFF; }
        .sf-btn-outline { display: inline-flex; align-items: center; justify-content: center; padding: 11px 24px; background: transparent; border: 1px solid rgba(158,27,27,.4); border-radius: 9999px; color: #f87171; font-family: 'Lexend', sans-serif; font-weight: 800; font-size: .75rem; letter-spacing: .1em; text-transform: uppercase; cursor: pointer; transition: border-color .2s, box-shadow .2s; }
        ...
        /* HERO */
        .sf-hero { position: relative; height: 100vh; min-height: 680px; display: flex; align-items: center; overflow: hidden; }
        ...
        .sf-hero-content { position: relative; z-index: 10; max-width: 1200px; margin: 0 auto; padding: 0 32px; width: 100%; }
        .sf-eyebrow { display: inline-flex; align-items: center; gap: 8px; background: rgba(158,27,27,.16); border: 1px solid rgba(158,27,27,.3); border-radius: 9999px; padding: 6px 16px; font-family: 'Lexend', sans-serif; font-weight: 700; font-size: .65rem; letter-spacing: .16em; text-transform: uppercase; color: #f87171; margin-bottom: 24px; }
        .sf-hero-title { font-family: 'Lexend', sans-serif; font-weight: 900; font-size: clamp(3rem, 8vw, 6rem); line-height: .96; letter-spacing: -.04em; color: #FFFFFF; margin-bottom: 22px; }
        .sf-accent { color: #9E1B1B; }
        .sf-hero-sub { font-size: clamp(.9rem, 1.6vw, 1.1rem); color: rgba(255,255,255,.65); line-height: 1.8; max-width: 520px; margin-bottom: 36px; }
        .sf-hero-btns { display: flex; gap: 14px; flex-wrap: wrap; margin-bottom: 18px; }
        .sf-hero-badge { display: inline-flex; align-items: center; background: rgba(255,255,255,.06); border: 1px solid rgba(255,255,255,.12); border-radius: 9999px; padding: 7px 18px; font-family: 'Lexend', sans-serif; font-weight: 700; font-size: .65rem; letter-spacing: .1em; text-transform: uppercase; color: rgba(255,255,255,.7); margin-bottom: 14px; }
        ...
        /* STATS */
        .sf-stats { background: #1a1a1a; border-top: 1px solid rgba(255,255,255,.06); border-bottom: 1px solid rgba(255,255,255,.06); }
        .sf-stats-inner { max-width: 1200px; margin: 0 auto; padding: 36px 32px; display: grid; grid-template-columns: repeat(4, 1fr); }
        .sf-stat-item { text-align: center; padding: 12px 20px; border-right: 1px solid rgba(255,255,255,.06); }
        .sf-stat-item:last-child { border-right: none; }
        .sf-stat-val { font-family: 'Lexend', sans-serif; font-weight: 900; font-size: clamp(1.8rem, 3vw, 2.8rem); letter-spacing: -.04em; color: #9E1B1B; line-height: 1; margin-bottom: 6px; }
        .sf-stat-label { font-size: .65rem; font-weight: 600; letter-spacing: .14em; text-transform: uppercase; color: rgba(255,255,255,.45); }

        /* LAYOUT */
        .sf-section { padding: clamp(72px, 10vw, 120px) 0; }
        .sf-section-alt { background: #1a1a1a; }
        .sf-container { max-width: 1200px; margin: 0 auto; padding: 0 32px; }
        ...
        .sf-section-title { font-family: 'Lexend', sans-serif; font-weight: 900; font-size: clamp(2rem, 4vw, 3.2rem); letter-spacing: -.04em; line-height: .96; color: #FFFFFF; margin-bottom: 16px; }
        .sf-section-sub { font-size: .95rem; color: rgba(255,255,255,.55); line-height: 1.8; max-width: 520px; margin: 0 auto; }
        ...
        .sf-role-img-wrap { position: relative; height: 260px; overflow: hidden; }
        ...
        .sf-role-title { font-family: 'Lexend', sans-serif; font-weight: 800; font-size: 1.3rem; letter-spacing: -.02em; color: #FFFFFF; margin-bottom: 10px; }
        .sf-role-desc { font-size: .85rem; color: rgba(255,255,255,.6); line-height: 1.75; margin-bottom: 20px; }
        .sf-role-list { list-style: none; display: flex; flex-direction: column; gap: 9px; margin-bottom: 4px; }
        .sf-role-list li { display: flex; align-items: center; gap: 10px; font-size: .82rem; color: rgba(255,255,255,.72); }
        .sf-check { color: #9E1B1B; font-weight: 700; flex-shrink: 0; }

        /* HOW IT WORKS */
        .sf-how-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 32px; position: relative; }
        .sf-how-card { background: #121212; border: 1px solid rgba(255,255,255,.06); border-radius: 16px; padding: 36px 28px; position: relative; }
        .sf-how-line { position: absolute; top: 54px; right: -17%; width: 34%; height: 0; border-top: 2px dashed rgba(178,42,39,.4); z-index: 1; }
        .sf-how-num { font-family: 'Lexend', sans-serif; font-weight: 900; font-size: clamp(2.8rem, 5vw, 4rem); letter-spacing: -.06em; color: #b22a27; line-height: 1; margin-bottom: 16px; }
        .sf-how-title { font-family: 'Lexend', sans-serif; font-weight: 800; font-size: 1.1rem; color: #FFFFFF; margin-bottom: 10px; }
        .sf-how-desc { font-size: .83rem; color: rgba(255,255,255,.55); line-height: 1.75; }

        /* FEATURES */
        .sf-features-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; }
        .sf-feat-card { background: #121212; border: 1px solid rgba(255,255,255,.06); border-radius: 14px; padding: 28px 24px; transition: border-color .3s, transform .3s, box-shadow .3s; }
        .sf-feat-card:hover { border-color: #b22a27; transform: scale(1.02); box-shadow: 0 0 24px rgba(178,42,39,.15); }
        .sf-feat-icon { font-size: 1.8rem; margin-bottom: 14px; display: block; }
        .sf-feat-title { font-family: 'Lexend', sans-serif; font-weight: 700; font-size: 1rem; color: #FFFFFF; margin-bottom: 10px; }
        .sf-feat-desc { font-size: .82rem; color: rgba(255,255,255,.55); line-height: 1.75; }

        /* CALCULATOR */
        .sf-calc-card { background: #1c1b1b; border-radius: 20px; padding: clamp(28px, 5vw, 48px); max-width: 680px; margin: 0 auto; }
        .sf-calc-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 20px; }
        .sf-calc-field label { display: block; font-size: .65rem; font-family: 'Lexend', sans-serif; font-weight: 700; letter-spacing: .14em; text-transform: uppercase; color: rgba(255,255,255,.45); margin-bottom: 8px; }
        .sf-calc-field input { width: 100%; background: #121212; border: 1px solid rgba(255,255,255,.08); border-radius: 8px; padding: 12px 14px; color: #FFFFFF; font-family: 'Inter', sans-serif; font-size: .9rem; outline: none; transition: border-color .2s; }
        .sf-calc-field input:focus { border-color: rgba(178,42,39,.5); }
        .sf-calc-toggles { display: flex; flex-wrap: wrap; gap: 20px; margin-bottom: 24px; }
        ...
        .sf-toggle-row { display: flex; gap: 6px; }
        .sf-toggle-btn { padding: 8px 16px; background: #121212; border: 1px solid rgba(255,255,255,.1); border-radius: 8px; color: rgba(255,255,255,.55); font-family: 'Lexend', sans-serif; font-size: .68rem; font-weight: 700; letter-spacing: .08em; text-transform: uppercase; cursor: pointer; transition: all .2s; }
        .sf-toggle-btn.active { background: #b22a27; border-color: #b22a27; color: #FFFFFF; }
        .sf-calc-select { background: #121212; border: 1px solid rgba(255,255,255,.08); border-radius: 8px; padding: 8px 14px; color: rgba(255,255,255,.7); font-family: 'Inter', sans-serif; font-size: .82rem; outline: none; cursor: pointer; }
        ...
        .sf-calc-kcal-val { font-family: 'Lexend', sans-serif; font-weight: 900; font-size: clamp(3rem, 8vw, 5rem); color: #b22a27; letter-spacing: -.06em; line-height: 1; }
        ...
        .sf-calc-fasting { display: flex; justify-content: space-between; align-items: center; background: rgba(178,42,39,.08); border: 1px solid rgba(178,42,39,.18); border-radius: 10px; padding: 14px 18px; margin-bottom: 20px; }
        ...
        /* PRICING */
        .sf-plans-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; align-items: start; margin-bottom: 24px; }
        .sf-plan-card { background: #1c1b1b; border: 1px solid rgba(255,255,255,.07); border-radius: 16px; padding: 32px 28px; position: relative; transition: transform .3s, box-shadow .3s; }
        ...
        .sf-plan-name { font-family: 'Lexend', sans-serif; font-weight: 900; font-size: .65rem; letter-spacing: .2em; text-transform: uppercase; color: rgba(255,255,255,.4); margin-bottom: 12px; }
        .sf-plan-price { font-family: 'Lexend', sans-serif; font-weight: 900; font-size: clamp(2rem, 4vw, 2.8rem); color: #FFFFFF; letter-spacing: -.04em; line-height: 1; margin-bottom: 24px; }
        .sf-plan-per { font-size: .85rem; font-weight: 400; color: rgba(255,255,255,.4); }
        .sf-plan-features { list-style: none; display: flex; flex-direction: column; gap: 10px; margin-bottom: 28px; }
        .sf-plan-features li { display: flex; align-items: center; gap: 10px; font-size: .82rem; color: rgba(255,255,255,.7); }
        .sf-plan-ghost-btn { width: 100%; padding: 12px; background: transparent; border: 1px solid rgba(255,255,255,.1); border-radius: 9999px; color: rgba(255,255,255,.6); font-family: 'Lexend', sans-serif; font-weight: 700; font-size: .68rem; letter-spacing: .1em; text-transform: uppercase; cursor: pointer; transition: border-color .2s, color .2s; }
        ...
        /* TESTIMONIALS */
        .sf-testi-layout { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; align-items: start; }
        .sf-testi-featured { background: linear-gradient(135deg, rgba(158,27,27,.1), rgba(158,27,27,.03)); border: 1px solid rgba(158,27,27,.2); border-radius: 20px; padding: clamp(28px, 4vw, 44px); position: relative; overflow: hidden; }
        .sf-testi-stat-big { font-family: 'Lexend', sans-serif; font-weight: 900; font-size: clamp(2rem, 4vw, 3rem); color: #b22a27; letter-spacing: -.04em; margin-bottom: 12px; line-height: 1; }
        ...
        .sf-quote-mark { font-family: 'Lexend', sans-serif; font-size: 8rem; font-weight: 900; color: rgba(158,27,27,.12); line-height: 1; position: absolute; top: -10px; left: 24px; pointer-events: none; }
        .sf-testi-text { font-family: 'Lexend', sans-serif; font-weight: 300; font-size: clamp(.95rem, 1.8vw, 1.3rem); line-height: 1.55; color: rgba(255,255,255,.88); font-style: italic; margin-bottom: 24px; position: relative; }
        .sf-testi-author { display: flex; align-items: center; gap: 12px; }
        .sf-testi-av { width: 44px; height: 44px; border-radius: 50%; object-fit: cover; border: 2px solid rgba(158,27,27,.3); flex-shrink: 0; }
        .sf-testi-name { font-weight: 600; font-size: .88rem; color: #FFFFFF; }
        .sf-testi-role { font-size: .7rem; color: rgba(255,255,255,.45); margin-top: 2px; }
        .sf-quote-stars { margin-left: auto; color: #9E1B1B; font-size: 1rem; letter-spacing: 2px; }
        .sf-testi-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
        .sf-testi-card { background: #1e1e1e; border: 1px solid rgba(255,255,255,.06); border-radius: 14px; padding: 22px; transition: border-color .2s; }
        .sf-testi-card:hover { border-color: rgba(178,42,39,.25); }
        .sf-testi-card-stat { font-family: 'Lexend', sans-serif; font-weight: 900; font-size: 1.6rem; color: #b22a27; letter-spacing: -.04em; margin-bottom: 10px; }
        .sf-testi-card-text { font-size: .8rem; color: rgba(255,255,255,.6); line-height: 1.7; font-style: italic; margin-bottom: 16px; }
        ...
        .sf-faq-q { width: 100%; display: flex; align-items: center; justify-content: space-between; gap: 20px; padding: 20px 24px; background: transparent; border: none; cursor: pointer; text-align: left; font-family: 'Inter', sans-serif; font-size: .88rem; font-weight: 500; color: #FFFFFF; line-height: 1.5; transition: color .2s; }
        .sf-faq-open .sf-faq-q { color: #f87171; }
        .sf-faq-icon { font-size: 1.2rem; color: #9E1B1B; flex-shrink: 0; line-height: 1; font-weight: 300; }
        .sf-faq-a { padding: 0 24px 20px; font-size: .83rem; color: rgba(255,255,255,.6); line-height: 1.8; }

        /* CTA */
        .sf-cta-section { position: relative; padding: clamp(80px, 12vw, 140px) 0; overflow: hidden; }
        ...
        .sf-cta-title { font-family: 'Lexend', sans-serif; font-weight: 900; font-size: clamp(2rem, 5vw, 3.8rem); letter-spacing: -.04em; line-height: .96; color: #FFFFFF; margin-bottom: 18px; }
        .sf-cta-sub { font-size: .95rem; color: rgba(255,255,255,.6); line-height: 1.8; max-width: 480px; margin: 0 auto 36px; }
        .sf-cta-btns { display: flex; gap: 14px; justify-content: center; flex-wrap: wrap; margin-bottom: 28px; }
        .sf-trust-row { display: flex; gap: 14px; justify-content: center; flex-wrap: wrap; }
        .sf-trust-badge { font-size: .65rem; color: rgba(255,255,255,.38); letter-spacing: .06em; }

        /* FOOTER */
        .sf-footer { background: #0e0e0e; border-top: 1px solid rgba(255,255,255,.05); padding: 56px 0 32px; }
        .sf-footer-inner { display: grid; grid-template-columns: 2fr 1fr 1fr; gap: 40px; margin-bottom: 48px; }
        ...
        .sf-footer-bottom { display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 8px; padding-top: 24px; border-top: 1px solid rgba(255,255,255,.05); font-size: .68rem; color: rgba(255,255,255,.25); letter-spacing: .04em; }
