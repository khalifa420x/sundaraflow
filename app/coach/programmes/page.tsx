...
type ViewType = 'list' | 'generate' | 'create' | 'assign' | 'tracking';
                ...
                <section style={{ position: 'relative', height: 'clamp(280px,40vh,440px)', overflow: 'hidden', flexShrink: 0 }}>
                  ...
                  <div style={{ position: 'relative', zIndex: 1, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: 'clamp(20px,4vw,48px)', opacity: mounted ? 1 : 0, transform: mounted ? 'none' : 'translateY(16px)', transition: 'opacity .6s ease, transform .6s ease' }}>
                    ...
                    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                      ...
                      <button style={S.ghostBtn} onClick={() => { setView('tracking'); fetchTracking(user); }}>📊 SUIVI CLIENTS</button>
                    </div>
                  </div>
                </section>
                          ...
                          {expandedGenSession === si && (
                            <div style={{ padding: '0 18px 18px' }}>
                              {session.exercises.map((ex, ei) => (
                                <div key={ei} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: '14px', marginBottom: 8 }}>
                                  ...
                                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 10 }}>
                                    ...
                                    <div>
                                      <div style={{ ...S.label, marginBottom: 5 }}>Reps/Durée</div>
                                      <input type="text" value={ex.reps} onChange={e => updateEditedEx(si, ei, { reps: e.target.value })} style={{ width: 72, background: '#2a2a2a', border: '1px solid rgba(178,42,39,0.3)', borderRadius: 6, padding: '6px 8px', color: '#b22a27', fontSize: '.78rem', outline: 'none', fontFamily: 'Lexend, sans-serif', fontWeight: 700 }} />
                                    </div>
                                    <div>
                                      <div style={{ ...S.label, marginBottom: 5 }}>Repos</div>
                            ...
                            <div key={ei} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: '14px', marginBottom: 8 }}>
                              <div style={{ display: 'flex', gap: 8, marginBottom: 10, alignItems: 'flex-start' }}>
                                <div style={{ flex: 1, position: 'relative' }}>
                                  <input type="text" placeholder="Nom de l'exercice" value={ex.name} list={`exn-${si}-${ei}`} onChange={e => updateCreateEx(si, ei, { name: e.target.value })} style={{ ...S.input, fontSize: '.78rem', padding: '7px 10px' }} />
                                  <datalist id={`exn-${si}-${ei}`}>{exNames.map(n => <option key={n} value={n} />)}</datalist>
                                </div>
                                <button
                                  onClick={() => { const copy = [...createSessions]; copy[si].exercises = copy[si].exercises.filter((_, idx) => idx !== ei); setCreateSessions([...copy]); }}
                                  style={{ width: 32, height: 32, background: 'rgba(220,38,38,0.1)', border: 'none', borderRadius: 7, cursor: 'pointer', color: '#f87171', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
                                >×</button>
                              </div>
                              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                                <div>
                                  <div style={{ ...S.label, marginBottom: 5 }}>Séries</div>
                                  <input type="number" value={ex.sets} min={1} onChange={e => updateCreateEx(si, ei, { sets: +e.target.value })} style={{ width: 64, background: '#2a2a2a', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 6, padding: '6px 8px', color: '#e5e2e1', fontSize: '.78rem', outline: 'none', fontFamily: 'Lexend, sans-serif', fontWeight: 700 }} />
                                </div>
                                <div>
                                  <div style={{ ...S.label, marginBottom: 5 }}>Reps</div>
                                  <input type="text" placeholder="12" value={ex.reps} onChange={e => updateCreateEx(si, ei, { reps: e.target.value })} style={{ width: 72, background: '#2a2a2a', border: '1px solid rgba(178,42,39,0.3)', borderRadius: 6, padding: '6px 8px', color: '#b22a27', fontSize: '.78rem', outline: 'none', fontFamily: 'Lexend, sans-serif', fontWeight: 700 }} />
                                </div>
                                <div>
                                  <div style={{ ...S.label, marginBottom: 5 }}>Repos</div>
                                  <input type="text" placeholder="60s" value={ex.rest} onChange={e => updateCreateEx(si, ei, { rest: e.target.value })} style={{ width: 72, background: '#2a2a2a', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 6, padding: '6px 8px', color: '#e5e2e1', fontSize: '.78rem', outline: 'none', fontFamily: 'Lexend, sans-serif' }} />
                                </div>
                                <div style={{ flex: 1, minWidth: 120 }}>
                                  <div style={{ ...S.label, marginBottom: 5 }}>Notes</div>
                                  <input type="text" placeholder="Conseils coach…" value={ex.notes} onChange={e => updateCreateEx(si, ei, { notes: e.target.value })} style={{ ...S.input, padding: '6px 8px', fontSize: '.76rem' }} />
                                </div>
                              </div>
                            </div>
                          ))}
                          <button onClick={() => { const copy = [...createSessions]; copy[si].exercises.push(defaultManualEx()); setCreateSessions([...copy]); }} style={{ ...S.ghostBtn, fontSize: '.62rem', padding: '6px 14px', marginTop: 4 }}>+ Ajouter un exercice</button>
                        </div>
                      )}
                    </div>
                  ))}
            ...
            {view === 'tracking' && (
              <div style={{ padding: 'clamp(16px,2.5vw,32px)', flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 30, flexWrap: 'wrap' }}>
                  <button style={{ ...S.ghostBtn, padding: '8px 14px' }} onClick={() => setView('list')}>← RETOUR</button>
                  <div>
                    <span style={S.tag}>📊 Suivi en temps réel</span>
                    <h2 style={{ fontFamily: 'Lexend, sans-serif', fontWeight: 900, fontSize: 'clamp(1.5rem,4vw,2.8rem)', letterSpacing: '-.04em', color: '#e5e2e1', margin: 0, lineHeight: .88 }}>
                      SUIVI <span style={{ color: '#b22a27', fontStyle: 'italic' }}>CLIENTS.</span>
                    </h2>
                  </div>
                </div>
                ...
                {trackingLoading ? (
                  ...
                  <div style={{ ...S.card, textAlign: 'center', padding: '40px 24px' }}>
                    ...
                    <div style={{ fontFamily: 'Lexend, sans-serif', fontWeight: 800, fontSize: '1rem', color: '#e5e2e1', marginBottom: 8 }}>Aucun client</div>
